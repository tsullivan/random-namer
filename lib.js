...
export class Renamer {
  ...
  attemptRename(original, responses) {
    ...
    return this.fs.exists().then(doesExist => {
      if (!doesExist) {
        return this.fs.rename(original, newName).then(() => newName);
      } else {
  ...
  rename({ files }) {
    ...
    return Promise.all(
      files.map(f => {
        ...
        return this.fs.isFile(f).then(result => {
      ...
      .then(result => {
        ...
        return responses
          .concat(`${result.length} ${noun} random-named`)
          .join('\n');
      });
  }
}
